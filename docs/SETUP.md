...
2-1. 이어서 같은 SQL Editor에서 `supabase/migrations/0002_ops_enhancements.sql`
   내용도 붙여넣고 실행합니다 (`source_system`/`resolved_at` 등 컬럼 추가,
   당직자·알림 테이블 생성). **이 파일을 실행하기 전에는 ERROR 로그로
   장애가 감지되는 순간 API가 500 에러를 반환합니다** — 새 마이그레이션이
   0001 다음에 실행되어야 합니다.
...
3. Deploy.
4. **Vercel Hobby(무료) 플랜 참고**: Hobby 플랜은 cron 작업이 하루 1회로
   제한됩니다 (그 이상 빈도로 설정하면 배포 자체가 실패합니다). `vercel.json`은
   이에 맞춰 하루 1회(`0 0 * * *`, UTC 자정)로 설정했습니다. 실행 빈도와
   무관하게 대시보드의 "지금 로그 생성" 버튼과 "자동 시뮬레이션(4초 간격)"
   토글은 언제나 즉시 동작하므로, 발표·데모는 이 버튼들로 진행하는 것을
   권장합니다.
...
- [ ] ERROR 로그 발생 시 "감지된 장애" 목록에 새 항목이 뜨는지 (500 에러가
      나면 마이그레이션 0002가 실행됐는지 먼저 확인)
- [ ] 장애 상세 페이지에서 체크리스트 체크 → 테스트 검증 → 시스템 복구까지
      순서대로 눌러 상태가 `resolved`로 바뀌는지
- [ ] 시스템 구조도의 노드가 장애 발생 시 색이 바뀌고, 클릭하면 로그/장애
      목록이 해당 시스템으로 필터링되는지
- [ ] 검색창 키워드 입력, 로그 레벨/위험도/발생 시스템/기간 필터가 실제로
      목록을 좁히는지, "필터 초기화"가 원상복구되는지
- [ ] 장애 상세 페이지의 "권장 조치 자동 실행" 승인 후 체크리스트 → 검증 →
      복구가 순서대로 자동 진행되는지
- [ ] 데이터 영향이 없는 장애에서 "장애 조치 완료 처리"로 바로 `resolved`가
      되는지
- [ ] 당직자 알림 패널에 "시뮬레이션 · 실제 발송 아님" 배지와 함께 발송
      기록이 보이는지 (실제 문자/메일은 발송되지 않음)
